...
- `/api/history` (public, CDN-cached 20s, same posture as `/api/data`) reads the dated snapshot history. Three query modes: `?date=YYYY-MM-DD` (one day), `?from=&to=` (inclusive range, oldest→newest), `?ticker=DDOG&days=30` (per-ticker `{date, price, changePct, rating?, sentiment}` series over the last N days). No UI yet — read layer for future mood timelines / streaks / per-stock charts.
...
- `@anthropic-ai/sdk` — two Claude calls remain, both Sonnet, each on its own cadence:
  - `claude-sonnet-4-6` for the OVERALL MOOD line (`lib/generateMood.ts`) — fires per `shouldRerunMood` (90-min floor, 4pp delta, plus four Stockholm checkpoints at 09:30 / 12:00 / 16:00 / 22:00 STO; weekday-only).
  - `claude-sonnet-4-6` for the Friday-evening WEEKLY CHAMPION recap (`lib/weeklyChampion.ts`) — once a week, long-form prose.
  - Per-ticker WSB comments are no longer generated by AI — they come from a static per-ticker × sentiment-bucket table in `lib/stockMessages.ts`. Owners are templated via `{owner}` and resolved at lookup time against the current `tickers.ts` ownership, so swapping a stock between users or adding a new friend auto-syncs every comment.
  - No web search; data is passed in; all three use native structured outputs via `output_config.format`.
- `@upstash/redis` — storage (Upstash Redis via Vercel Marketplace; keys `iskbets:snapshot` for the live data, `iskbets:lastAttempt` for the cooldown gate, `iskbets:snapshot:YYYY-MM-DD` + `iskbets:snapshot:index` for dated history)
...
/api/trigger (GET; manual or scheduled — `x-trigger-secret: ${TRIGGER_SECRET}` header, or legacy ?key=)
  ...
  → maybeSaveDailySnapshot() (every run; computeDailySnapshot() → saveDailySnapshot() writes iskbets:snapshot:YYYY-MM-DD + registers it in the index sorted set, then trims to the 400-day cap. Fully try/caught — a failure here never breaks the run.)
...
/api/history (GET, public, CDN-cached 20s)
  → ?date=YYYY-MM-DD               getDailySnapshot()          → DailySnapshot | 404
  → ?from=&to=                     getDailySnapshotsInRange()  → { from, to, count, snapshots[] } (oldest→newest)
  → ?ticker=DDOG&days=30           getRecentDailySnapshots() + extractTickerSeries() → { ticker, days, count, series[] }
```
...
Ten fixed Redis keys (two are hashes) plus one dated-history family, all under the `iskbets:` namespace:
...
// iskbets:snapshot:YYYY-MM-DD — dated end-of-day history. One slim
//                   DailySnapshot per Stockholm calendar day, written by
//                   maybeSaveDailySnapshot() on EVERY trigger — the last
//                   write of the day wins (end-of-day state, not intraday
//                   noise). Per ticker: { ticker, price, changePct,
//                   rating?, sentiment, comment? }; plus overallMood (the
//                   day's "brief"), updatedAt, capturedAt. ~5 KB/day
//                   (the live snapshot is ~23 KB — the heavy volume /
//                   52-week / marketState fields and the pricesAtLastMood
//                   duplicate are dropped). Read via getDailySnapshot /
//                   getDailySnapshotsInRange / getRecentDailySnapshots.
//                   Surfaced through /api/history. See lib/dailySnapshot.ts
//                   for the projection.

// iskbets:snapshot:index — Redis SORTED SET. member = date (YYYY-MM-DD),
//                   score = YYYYMMDD (chronological). Lets history be
//                   enumerated / range-queried without KEYS/SCAN.
//                   RETENTION: capped at 400 days — on each write, any
//                   dates beyond the cap are deleted (key + index member,
//                   oldest first). Normally trims exactly one date/day
//                   once the corpus is full. ~2 MB of history at the cap.
