...
- `@anthropic-ai/sdk` — three Claude calls, each on its own cadence:
  ...
  - `claude-sonnet-4-6` for the OVERALL MOOD line (`lib/generateMood.ts`) — fires per `shouldRerunMood` (90-min floor, 4pp delta, plus three Stockholm checkpoints at 09:30 / 16:00 / 22:00 STO; weekday-only). Split out from the comments call because Haiku tended to enumerate every ticker into one long mood sentence.
